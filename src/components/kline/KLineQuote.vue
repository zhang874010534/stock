...
const weekday = computed(() => props.quote?.date ? new Intl.DateTimeFormat('zh-CN', { weekday: 'long', timeZone: 'UTC' }).format(new Date(`${props.quote.date}T00:00:00Z`)) : '—')
const amplitude = computed(() => {
  const q = props.quote
  const previousClose = q && Number.isFinite(q.change) ? q.close - q.change : null
  return previousClose > 0 ? (q.high - q.low) / previousClose * 100 : null
})
...
<template>
  <div class="kline-quote" :class="{ 'is-floating': floating, 'on-left': side === 'left' }" :style="{ '--quote-offset': `${overlayOffset}px` }" aria-label="当前 K 线行情">
    ...
    <div class="quote-date"><time>{{ quote?.date ?? '—' }}</time><span>{{ floating ? weekday : isLatest ? '最新' : '历史' }}</span><span v-if="quote?.startDate && quote.startDate !== quote.endDate" class="period-span">{{ quote.startDate }} 起</span></div>
    <div class="quote-values">
      ...
      <span v-if="floating" title="相对上一根同周期收盘价">振幅 <b>{{ signed(amplitude, '%') }}</b></span>
...
.is-floating { position: absolute; inset: 0; z-index: 7; padding: 0; pointer-events: none; }
.is-floating .quote-details { position: absolute; top: var(--quote-offset); right: 16px; width: 184px; max-width: calc(100% - 80px); padding: 6px 8px; border: 1px solid #315376; background: #11151edb; pointer-events: none; }
